...
class BankingApp:
    def __init__(self, root):
        ...
        self.create_account_button = tk.Button(
            root, text="Create Account", command=self.create_account
        )
        self.create_account_button.pack(pady=5)

        self.deposit_button = tk.Button(
            root, text="Deposit", command=self.deposit, state=tk.DISABLED
        )
        self.deposit_button.pack(pady=5)

        self.withdraw_button = tk.Button(
            root, text="Withdraw", command=self.withdraw, state=tk.DISABLED
        )
        self.withdraw_button.pack(pady=5)

        self.check_balance_button = tk.Button(
            root, text="Check Balance", command=self.check_balance, state=tk.DISABLED
        )
        self.check_balance_button.pack(pady=5)

    def create_account(self):
        account_holder = simpledialog.askstring(
            "Create Account", "Enter account holder's name:"
        )
        if account_holder is not None:
            account_number = simpledialog.askinteger(
                "Create Account", "Enter account number:"
            )
            if account_number is not None:
                initial_balance = simpledialog.askfloat(
                    "Create Account", "Enter initial balance:"
                )
    ...
    def deposit(self):
        account_number = simpledialog.askinteger(
            "Deposit", "Enter your account number:"
        )
        if account_number is not None:
            ...
            if account:
                ...
                if amount is not None:
                    if account.deposit(amount):
                        ...
                        messagebox.showwarning(
                            "Invalid Amount", "Please enter a valid deposit amount."
                        )
            else:
    ...
    def withdraw(self):
        account_number = simpledialog.askinteger(
            "Withdraw", "Enter your account number:"
        )
        if account_number is not None:
            account = self.accounts.get(account_number)
            if account:
    ...
    def check_balance(self):
        account_number = simpledialog.askinteger(
            "Check Balance", "Enter your account number:"
        )
        if account_number is not None:
            account = self.accounts.get(account_number)
            if account:
    ...
    def update_account_info_label(self):
        account_number = simpledialog.askinteger(
            "Update Account Info", "Enter your account number:"
        )
        if account_number is not None:
            account = self.accounts.get(account_number)
            if account:
                self.account_info_label.config(
                    text="Account Holder: {} | Balance: ${}".format(
                        account.account_holder, account.balance
                    )
                )
            else:
